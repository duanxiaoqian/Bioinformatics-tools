...
# data <- read.csv("ex3.csv")
ThreeD_scatterplot(path=path, filename = filename, symbol=FALSE)
ThreeD_scatterplot <- function(path=path, filename=filename, symbol=FALSE){
  ...
  for (i in sheetnames){
    ...
    colors <- sample(c('#33CC00','#FF0033','#0099FF','#FF9900','#CC00FF',
                       '#99FF66','#FF6600','#33FFFF','#FFCC33','#FF33FF',
                       '#336600','#FF6633','#0000FF','#FFFF00','#9900FF'), groupcount)#green,red, blue,yellow,purple
    if (symbol){
      replace <- ifelse(groupcount > 8, T, F)
      symbol <- data$Group
      symbols <- sample(c("circle","square","diamond",
                          "circle-open","square-open",
                          "diamond-open","cross","x"
                          ),groupcount,replace = replace)
    }else{
      symbol <- NULL
      symbols <- "circle"
    }
